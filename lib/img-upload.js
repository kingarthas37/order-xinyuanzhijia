...
module.exports = function(req,callback) {
    ...
    form.parse(req,function(err, fields, files) {
        ...
        var isFile = req.query['is-file'];
        
        if(imageFile.size !== 0){
            
            if(!isFile && imageFile.headers['content-type'] !== 'image/gif' && imageFile.headers['content-type'] !== 'image/jpeg' && imageFile.headers['content-type'] !=='image/png'  ) {
                return callback({
                    success:0,
                    error:'错误的图片文件格式'
                });
